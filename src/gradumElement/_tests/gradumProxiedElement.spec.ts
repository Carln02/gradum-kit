import {describe, it, expect} from "vitest";
import {GradumProxiedElement} from "../gradumProxiedElement/gradumProxiedElement";
import {div} from "../../elementCreation/basicElements";
import {gradum} from "../../gradumFunctions/gradumFunctions";

class TE extends GradumProxiedElement<"div"> {
}

describe("GradumProxiedElement: selected toggling", () => {
    it("adds/removes the default selected class on the wrapped element", () => {
        const p = TE.create({tag: "div"});
        gradum(p).defaultSelectedClasses = "mySel";
        gradum(p).selected = true;
        expect(p.element.classList.contains("mySel")).toBe(true);

        gradum(p).selected = false;
        expect(p.element.classList.contains("mySel")).toBe(false);
    });

    it("plays nice when element is created via helper (div()) and then proxied", () => {
        const host = div({id: "h"});
        document.body.appendChild(host);

        const p = (class extends GradumProxiedElement<"div"> {}).create({tag: "div"});
        document.body.appendChild(p.element);

        gradum(p).selected = true;
        expect(p.element.classList.contains("selected")).toBe(true);
        expect(host.classList.contains("selected")).toBe(false);
    });
});