...
function makeWheel(values: string[] = []): GradumSelectWheel<string> {
    const wheel = GradumSelectWheel.create({}) as GradumSelectWheel<string>;
...
describe("GradumSelectWheel — entry drag listener (Bug 1: behavior propagation)", () => {
    it("each entry added via values has a dragStart on() listener", () => {
        const wheel = makeWheel(["A", "B", "C"]);
        for (const entry of wheel.entries) {
            expect(gradum(entry).hasListenersByType("gradum-drag-start")).toBe(true);
        }
    });
    ...
    it("wheel element itself has no dragStart listener (listeners are on entries)", () => {
        ...
        expect(gradum(wheel as unknown as Node).hasListenersByType("gradum-drag-start")).toBe(false);
    });
    ...
    it("entry dragStart listener fires on executeAction", () => {
        ...
        expect(() => gradum(entry).executeAction("gradum-drag-start", undefined, new Event("gradum-drag-start"))).not.toThrow();
    });
});

// ─── reloadEntrySizes retry (Bug 2 — zero-size layout) ────────────────────────

describe("GradumSelectWheel — reloadEntrySizes zero-size retry (Bug 2)", () => {
...
describe("GradumSelectWheel — removeEntry", () => {
...
describe("GradumSelectWheel — position/index math with known sizes", () => {
    let rafSpy: ReturnType<typeof vi.spyOn>;
    ...
    function makeWheelWithSizes(sizes: number[]): GradumSelectWheel<string> {
