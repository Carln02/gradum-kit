...
class GradumNodeList<Type extends object = object> {
    ...
    private subNodeListHandlers: Map<GradumNodeList<Type>, (entry: Type, state: "added" | "removed") => void> = new Map();
    ...
    protected* resolveSlot(slot: WeakRef<NodeListSlot<Type>>): IterableIterator<Type> {
        ...
        if (!obj) return;
        if (this.isGradumNodeList(obj)) yield* obj;
    ...
    protected findContainingSlot(entry: Type): NodeListSlot<Type> {
        for (const slot of this.slots) {
            ...
            if (this.isGradumNodeList(obj) && obj.has(entry)) return obj;
