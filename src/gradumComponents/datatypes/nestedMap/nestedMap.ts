...
class GradumNestedMap<ValueType = any, KeyType = string | symbol | number> {
    ...
    public get(...keys: KeyType[]): ValueType {
        ...
        for (const key of keys) {
            if (!(node instanceof GradumNestedMapNode)) return;
            node = node.get(key);
        }
        return node;
    }
    ...
    public set(value: ValueType, ...keys: KeyType[]) {
        ...
        for (let i = 0; i < keys.length - 1; i++) {
            const key = keys[i];
            if (!node.has(key) || !(node.get(key) instanceof GradumNestedMapNode)) node.set(key, new GradumNestedMapNode());
            node = node.get(key);
        }
    ...
    public has(...keys: KeyType[]): boolean {
        ...
        if (!(parent instanceof GradumNestedMapNode)) return false;
    ...
    public remove(...keys: KeyType[]): void {
        if (!keys.length) return;
        const parent = this.get(...keys.slice(0, -1));
        if (parent instanceof GradumNestedMapNode) parent.delete(keys[keys.length - 1]);
    }
    ...
    protected findPaths(
        ...
        if (!node || !(node instanceof GradumNestedMapNode)) return [];
