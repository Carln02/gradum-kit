...
import {TurboMovable} from "../../../turboComponents/wrappers/movable/movable";
...
describe("TurboSelector.feedforward — positioning wrapper (wrap: true)", () => {
    ...
    function makeWrapper(origin: HTMLElement): TurboMovable {
        const wrapper = turbo(origin).feedforward({wrap: true, removeOnPointerRelease: false}) as unknown as TurboMovable;
        document.body.appendChild(wrapper as unknown as Node);
        return wrapper;
    }

    it("returns a TurboMovable containing the clone", () => {
        const wrapper = makeWrapper(makeOrigin());

        expect(wrapper).toBeInstanceOf(TurboMovable);
        expect(wrapper.classList.contains("turbo-feedforward-wrapper")).toBe(true);
        expect(wrapper.children.length).toBe(1);
        expect(wrapper.firstElementChild.classList.contains("turbo-feedforward-clone")).toBe(true);
    });
    ...
    it("injects the transform-neutralizing stylesheet exactly once", () => {
        ...
        const sheets = document.querySelectorAll("#turbo-feedforward-styles");
    ...
    it("repeated feedforward() calls reuse the same wrapper", () => {
        ...
        const first = turbo(origin).feedforward({wrap: true, removeOnPointerRelease: false});
        const second = turbo(origin).feedforward({wrap: true, removeOnPointerRelease: false});
        expect(second).toBe(first);
    });

    it("without wrap, the clone itself is returned (no wrapper)", () => {
        const origin = makeOrigin();
        const clone = turbo(origin).feedforward({removeOnPointerRelease: false, type: "unwrapped"}) as any;
        expect(clone).not.toBeInstanceOf(TurboMovable);
