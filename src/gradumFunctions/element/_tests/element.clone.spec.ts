...
class NoteModel extends GradumModel {}
class NoteView extends GradumView {}
class NoteOperator extends GradumOperator {}

@define("CloneCard")
class CloneCard extends GradumElement {
...
class YjsNoteModel extends GradumYModel {}

@define("YjsCard")
class YjsCard extends GradumElement {
    static defaultProperties = {
        model: YjsNoteModel,
    };
}
...
describe("GradumSelector.clone — vanilla element", () => {
    ...
    it("copies id and class attributes", () => {
        ...
        const clone = gradum(div).clone() as HTMLDivElement;
    ...
    it("does not throw when element has no attributes", () => {
        ...
        expect(() => gradum(span).clone()).not.toThrow();
    });
});

describe("GradumSelector.clone — type and identity (CloneCard)", () => {
...
describe("GradumSelector.clone — primitive and plain fields", () => {
...
describe("GradumSelector.clone — object and array fields", () => {
...
describe("GradumSelector.clone — node fields", () => {
...
describe("GradumSelector.clone — Delegate safety", () => {
...
describe("GradumSelector.clone — getter-only and read-only fields", () => {
...
describe("GradumSelector.clone — options: exclude / forceInclude / copyReference", () => {
...
describe("GradumSelector.clone — MVC", () => {
    ...
    it("clone has an operator of the same type", () => {
        const origin = makeCard();
        const clone = origin.clone() as CloneCard;
        const cloneOps: GradumOperator[] = (clone as any).operators;
    ...
    it("clone operator instance is different from origin's", () => {
        const origin = makeCard();
        const clone = origin.clone() as CloneCard;
        const originOps: GradumOperator[] = (origin as any).operators;
        const cloneOps: GradumOperator[] = (clone as any).operators;
        expect(cloneOps[0]).not.toBe(originOps[0]);
    });
});

describe("GradumSelector.clone — Yjs (GradumYModel)", () => {
    ...
    it("snapshotData: true gives the clone a real detached Y.Map in a fresh doc", () => {
        const {el, ymap, doc} = makeYjsCard();
        const clone = el.clone({snapshotData: true}) as YjsCard;
        const cloneData = (clone as any).data;

        expect(cloneData).not.toBe(ymap);
        expect(cloneData).toBeInstanceOf(Y.Map);
        expect(cloneData.doc).toBeTruthy();
        expect(cloneData.doc).not.toBe(doc);
        expect(cloneData.get("title")).toBe("Yjs Note");
        expect(cloneData.get("count")).toBe(7);
    });

    it("snapshotData: true — mutations on the origin do not reach the clone (and vice versa)", () => {
        const {el, ymap} = makeYjsCard();
        const clone = el.clone({snapshotData: true}) as YjsCard;
        const cloneData = (clone as any).data;

        ymap.set("title", "Origin change");
        expect(cloneData.get("title")).toBe("Yjs Note");

        cloneData.set("title", "Clone change");
        expect(ymap.get("title")).toBe("Origin change");
    });
});

describe("GradumSelector.clone — edge cases", () => {
    it("cloning a plain div never throws", () => {
        const div = document.createElement("div");
        expect(() => gradum(div).clone()).not.toThrow();
    });

    it("returns undefined for a non-Node element (e.g. plain object)", () => {
        const obj = {};
        const result = gradum(obj as any).clone();
    ...
    it("does not share internal MVC collections between origin and clone", () => {
        ...
        gradum(origin).addOperator(extraOp);
        const cloneOps: GradumOperator[] = (clone as any).operators;
    ...
    it("cloning an element with no MVC set works without throwing", () => {
        ...
        expect(() => gradum(bare).clone()).not.toThrow();
        const clone = gradum(bare).clone() as HTMLDivElement;
