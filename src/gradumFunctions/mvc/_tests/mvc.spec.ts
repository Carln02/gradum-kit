...
class MyModel extends GradumModel<TestData> {}

class MyView extends GradumView {
    public inits = 0;
    public override initialize(): void { super.initialize(); this.inits++; }
}

class MyOperator extends GradumOperator {
    public inits = 0;
    public override initialize(): void { super.initialize(); this.inits++; }
}

class MyHandler extends GradumHandler {}

class MyInteractor extends GradumInteractor {
    public inits = 0;
    public override initialize(): void { super.initialize(); this.inits++; }
}

class MyTool extends GradumTool {
    public inits = 0;
    public override initialize(): void { super.initialize(); this.inits++; }
}

class MyConstrainer extends GradumConstrainer {
    public inits = 0;
    public override initialize(): void { super.initialize(); this.inits++; }
}
...
describe("GradumSelector MVC functions", () => {
    ...
    describe("model getter / setter", () => {
        ...
        it("model setter from constructor creates a GradumModel instance", () => {
            const s = sel();
            s.model = MyModel;
    ...
    describe("emitter getter / setter", () => {
        ...
        it("emitter setter from constructor creates a GradumEmitter instance", () => {
            const s = sel();
            s.emitter = GradumEmitter;
            expect(s.emitter).toBeInstanceOf(GradumEmitter);
        });

        it("emitter setter from instance stores that exact instance", () => {
            const s = sel();
            const e = new GradumEmitter();
            s.emitter = e;
    ...
    describe("setMvc", () => {
        it("creates model, view, and emitter from constructors", () => {
            const s = sel();
            s.setMvc({model: MyModel, view: MyView, emitter: GradumEmitter, initialize: false});
            expect(s.model).toBeInstanceOf(MyModel);
            expect(s.view).toBeInstanceOf(MyView);
            expect(s.emitter).toBeInstanceOf(GradumEmitter);
        });

        it("creates a default GradumEmitter when none is provided", () => {
            const s = sel();
    ...
    describe("getMvcDifference", () => {
        it("returns an empty object when configuration matches exactly", () => {
            const s = sel();
            s.setMvc({model: MyModel, view: MyView, initialize: false});
            const diff = s.getMvcDifference({model: MyModel, view: MyView, emitter: GradumEmitter});
    ...
    describe("addOperator / getOperator / removeOperator", () => {
        ...
        it("addOperator wires operator to the mvc model and emitter", () => {
            const s = sel();
            s.setMvc({model: MyModel, emitter: GradumEmitter, initialize: false});
            const ctrl = new MyOperator({element: s.element} as any);
            ctrl.keyName = "wired";
            s.addOperator(ctrl);
    ...
    describe("cross-wiring", () => {
        ...
        it("model changes propagate to the emitter via fireKey", () => {
            const s = sel();
            s.setMvc({model: MyModel, emitter: GradumEmitter, data: {foo: 0}, initialize: true});
