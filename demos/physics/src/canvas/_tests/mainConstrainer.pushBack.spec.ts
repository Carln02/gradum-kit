...
//A movable stand-in: a real position the solver can write to, and a box derived from it.
function movable(x: number, y: number, size = 100, angleDeg = 0) {
    const el: any = Object.create(Element.prototype);
    el.position = new Point(x, y);
    el.getBoundingClientRect = () => new GradumRect(
        {x: el.position.x, y: el.position.y, width: size, height: size, angleDeg});
    return el;
}
