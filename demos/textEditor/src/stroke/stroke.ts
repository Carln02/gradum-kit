import {Point, GradumHeadlessElement} from "../../../../build/gradum-kit.esm";
...
export class Stroke extends GradumHeadlessElement {
    /**
     * @description Where the passage starts in the document. Fixed for the stroke.
     */
    public from: number;
    ...
    /**
     * @description Whether the passage's items are words or characters.
     */
    public unit: TextUnit;
    ...
    public static customCreate(properties: StrokeProperties): any {
        const start = properties.coordsAt(0);
        const end = properties.coordsAt(properties.text.length);

        const unit = /\s/.test(properties.text) ? "word" : "char";
        const parts = unit === "word" ? properties.text.split(/(\s+)/) : [...properties.text];
        const tokens = unit === "word" ? parts.filter((_, index) => index % 2 === 0) : parts;

        const head = properties.coordsAt(tokens[0].length);
        const character = Math.max(3, Math.abs(head.right - start.left) / tokens[0].length);

        return super.customCreate({
            ...properties,
            original: properties.text,
            length: properties.text.length,
            unit: /\s/.test(properties.text) ? "word" : "char",

            tokens,
            pivot: new Point((start.left + end.right) / 2, (start.top + end.bottom) / 2),
            gaps: unit === "word" ? parts.filter((_, index) => index % 2 === 1) : [],
            step: Math.max(6, character * properties.text.length / tokens.length),
